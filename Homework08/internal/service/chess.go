...
func HandleGameProcess(
	...
	for {
		...
		move, err := HandlePlayerInput(ctx, g.GetBoard(), currentPlayer, buffer)
		if err != nil {
...
func RunConcurrentGame(
	...
	if g == nil {
		sendResult(ctx, results, model.CommandResult{NumberOfBoard: numberOfBoard, Finished: true, Err: errors.New("game is nil")})
		return
	}

	random := rand.New(rand.NewSource(time.Now().UnixNano()))
	sendStateToDrawer(ctx, g, drawer, numberOfBoard, "", 0, 0, false)

	for {
		select {
		case <-ctx.Done():
			log.Println("Получен сигнал об окончании работы.")
			return
		case command, ok := <-commands:
			if !ok {
				log.Println("channel with commands is closed")
				return
			}
			switch command.Type {
			case model.GiveUP:
				player := g.GetCurrentPlayer()
				sendStateToDrawer(ctx, g, drawer, numberOfBoard, player.GetPlayerName(), 0, 0, true)
				sendResult(ctx, results, model.CommandResult{NumberOfBoard: numberOfBoard, Finished: true})
				return
			case model.Auto:
				err := makeAutoMoves(g, command.AutoMoveCount, random, time.Sleep, func(player string, duration time.Duration, remaining int) {
					sendStateToDrawer(ctx, g, drawer, numberOfBoard, player, duration, remaining, false)
				})
				sendResult(ctx, results, model.CommandResult{NumberOfBoard: numberOfBoard, Err: err})
			default:
				sendResult(ctx, results, model.CommandResult{
...
func sendStateToDrawer(
	...
	case <-ctx.Done():
		return
	case drawer <- snapshot:

	}
}

func HandlePlayerInput(ctx context.Context, b *model.Board, player *model.Player, scanner *bufio.Scanner) (*model.GameMove, error) {
	move, input, err := console.ReadAndConvertPlayerInput(ctx, scanner, console.ReadFromStdIn)
	if err != nil {
